...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f69mX = new float[min];
            this.f70mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f69mX[i2] = fArr[0];
                this.f70mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f69mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f70mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f69mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f70mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f69mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f69mX[0]);
            sb.append(CSVUtil.COLUMN_SEPARATOR);
            sb.append(this.f70mY[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f69mX[i6]);
            sb.append(CSVUtil.COLUMN_SEPARATOR);
            sb.append(this.f70mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f69mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f69mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f69mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f70mY[i];
        }
        float[] fArr2 = this.f70mY;
        float f3 = fArr2[i];
