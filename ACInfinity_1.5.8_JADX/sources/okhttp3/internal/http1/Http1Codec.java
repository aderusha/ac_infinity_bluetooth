...
public final class Http1Codec implements HttpCodec {
    ...
    public void writeRequest(Headers headers, String str) throws IOException {
        if (this.state == 0) {
            this.sink.writeUtf8(str).writeUtf8(CSVUtil.ROW_SEPARATOR);
            int size = headers.size();
            for (int i = 0; i < size; i++) {
                this.sink.writeUtf8(headers.name(i)).writeUtf8(": ").writeUtf8(headers.value(i)).writeUtf8(CSVUtil.ROW_SEPARATOR);
            }
            this.sink.writeUtf8(CSVUtil.ROW_SEPARATOR);
            this.state = 1;
            return;
        }
    ...
    private final class ChunkedSink implements Sink {
        ...
        public void write(Buffer buffer, long j) throws IOException {
            if (this.closed) {
                ...
                Http1Codec.this.sink.writeUtf8(CSVUtil.ROW_SEPARATOR);
                Http1Codec.this.sink.write(buffer, j);
                Http1Codec.this.sink.writeUtf8(CSVUtil.ROW_SEPARATOR);
            }
        }
