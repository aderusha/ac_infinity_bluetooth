...
import com.eternal.notification.p008ui.AutomationFragmentV4;
...
public class AutomationModelV4 extends BaseViewModel {
    ...
    public MutableLiveData<Boolean> day1 = new MutableLiveData<>();
    public MutableLiveData<Boolean> day2 = new MutableLiveData<>();
    public MutableLiveData<Boolean> day3 = new MutableLiveData<>();
    public MutableLiveData<Boolean> day4 = new MutableLiveData<>();
    public MutableLiveData<Boolean> day5 = new MutableLiveData<>();
    public MutableLiveData<Boolean> day6 = new MutableLiveData<>();
    public MutableLiveData<Boolean> day7 = new MutableLiveData<>();
    public String devId;
    ...
    public BindingCommand<String> nameChanged = new BindingCommand<>(new BindingConsumer<String>() {
        public void call(String str) {
            MutableLiveData<String> mutableLiveData = AutomationModelV4.this.nameSuggest;
    ...
    /* renamed from: on */
    public MutableLiveData<Boolean> f216on = new MutableLiveData<>();
    ...
    public BindingCommand<Void> onConfirm = new BindingCommand<>((BindingAction) new BindingAction() {
        public void call() {
            if (!RepositoryInjection.providerDeviceRepository().isConnect(AutomationModelV4.this.notification.mac)) {
                AutomationModelV4 automationModelV4 = AutomationModelV4.this;
                automationModelV4.showFailDialog(Utils.getString(ProtocolTransformer.isWorkWiFi(automationModelV4.deviceType, AutomationModelV4.this.connectType) ? C2663R.string.tip_wifi_disconnect : C2663R.string.tip_ble_disconnect));
            } else if (AutomationModelV4.this.saveDis != null && !AutomationModelV4.this.saveDis.isDisposed()) {
            } else {
                if (AutomationModelV4.this.name.getValue() == null || TextUtils.isEmpty(AutomationModelV4.this.name.getValue().trim())) {
                    AutomationModelV4.this.showFailDialog(Utils.getString(C2420R.string.tip_empty_name));
                    AutomationModelV4.this.name.setValue(AutomationModelV4.this.originName);
                } else if (ProtocolTransformer.isWorkWiFi(AutomationModelV4.this.deviceType, AutomationModelV4.this.connectType)) {
                    AutomationModelV4.this.saveForNet();
                } else {
                    AutomationModelV4.this.saveForBle();
                }
            }
        }
    });
    ...
    public AutomationModelV4(Application application) {
        super(application);
        ...
        if (this.disClickable.getValue() != Boolean.TRUE) {
            ...
            observable.subscribeOn(Schedulers.m983io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new DataObserver<Void>() {
                public void doOnSubscribe(Disposable disposable) {
                    AutomationModelV4.this.saveDis = disposable;
                    AutomationModelV4.this.showDialog((String) null, disposable);
                }

                /* access modifiers changed from: protected */
                public void onError(String str) {
                    RxBus.getDefault().post(new ProgressEvent((byte) 1));
                    AutomationModelV4.this.showFailDialog(str);
                    AutomationModelV4.this.disClickable.setValue(false);
                }

                /* access modifiers changed from: protected */
                public void onSuccess(Void voidR) {
                    Notification unused = AutomationModelV4.this.notification = create;
                    AutomationModelV4.this.repository.setNotification(AutomationModelV4.this.notification, z);
                    RxBus.getDefault().post(new ProgressEvent((byte) 1));
                    AutomationModelV4.this.saveDis = Maybe.just(true).delay(1500, TimeUnit.MILLISECONDS).subscribe(new Consumer<Boolean>() {
                        public void accept(Boolean bool) throws Exception {
                            AutomationModelV4.this.back();
                            AutomationModelV4.this.dismissDialog();
                        }
                    });
                }
            });
    ...
    public void init(NotificationRepository notificationRepository, String str, String str2, byte b, byte b2, byte b3, Notification notification2, boolean z, byte b4, byte b5, int i, byte b6) {
        this.devId = str2;
    ...
    private void init(Notification notification2) {
        ...
        Messenger.getDefault().sendNoMsg(AutomationFragmentV4.UPDATE_PARAM);
    }
    ...
    private void initTime(Notification notification2) {
        ...
        this.allDay.set(!ByteUtils.getBit(notification3.day, 0));
        this.day7.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.day, 1)));
        this.day6.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.day, 2)));
        this.day5.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.day, 3)));
        this.day4.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.day, 4)));
        this.day3.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.day, 5)));
        this.day2.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.day, 6)));
        this.day1.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.day, 7)));
        this.port1.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.port, 7)));
        this.port2.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.port, 6)));
        this.port3.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.port, 5)));
        this.port4.setValue(Boolean.valueOf(!ByteUtils.getBit(notification3.port, 4)));
    }
    ...
    private Notification create() {
        ...
        byte b = 0;
        byte b2 = this.allDay.get() ? (byte) 128 : 0;
        if (this.day7.getValue() == Boolean.TRUE) {
            b2 = (byte) (b2 | SignedBytes.MAX_POWER_OF_TWO);
        }
        if (this.day6.getValue() == Boolean.TRUE) {
            b2 = (byte) (b2 | 32);
        }
        if (this.day5.getValue() == Boolean.TRUE) {
            b2 = (byte) (b2 | Ascii.DLE);
        }
        if (this.day4.getValue() == Boolean.TRUE) {
            b2 = (byte) (b2 | 8);
        }
        if (this.day3.getValue() == Boolean.TRUE) {
            b2 = (byte) (b2 | 4);
        }
        if (this.day2.getValue() == Boolean.TRUE) {
            b2 = (byte) (b2 | 2);
        }
        if (this.day1.getValue() == Boolean.TRUE) {
            b2 = (byte) (b2 | 1);
        }
        clone.day = b2;
    ...
    private void removeForBle() {
        BleStatue connect = this.repository.getConnect(this.notification.mac);
        if (connect != null) {
            this.removeDis = this.repository.removeNotification(connect, this.notification, true).compose(RxUtils.bindToLifecycle(getLifecycleProvider())).subscribeOn(Schedulers.m983io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<Boolean>() {
    ...
    private void removeForNet() {
        ApiHelper.getDeviceApi().delADVInfo(this.devId, this.notification.advId).subscribeOn(Schedulers.m983io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new DataObserver<Void>() {
            public void doOnSubscribe(Disposable disposable) {
                AutomationModelV4.this.removeDis = disposable;
                AutomationModelV4.this.showDialog((String) null, disposable);
                RxBus.getDefault().post(new ProgressEvent((byte) 0));
            }
