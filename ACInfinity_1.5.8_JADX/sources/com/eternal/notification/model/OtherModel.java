...
import com.eternal.notification.p008ui.OtherFragment;
...
public class OtherModel extends BaseViewModel {
    ...
    public BindingCommand<String> nameChanged = new BindingCommand<>(new BindingConsumer<String>() {
        public void call(String str) {
            MutableLiveData<String> mutableLiveData = OtherModel.this.nameSuggest;
    ...
    public BindingCommand<Void> onConfirm = new BindingCommand<>((BindingAction) new BindingAction() {
        public void call() {
            if (!RepositoryInjection.providerDeviceRepository().isConnect(OtherModel.this.notification.mac)) {
                OtherModel otherModel = OtherModel.this;
                otherModel.showFailDialog(Utils.getString(ProtocolTransformer.isWorkWiFi(otherModel.deviceType, OtherModel.this.connectType) ? C2663R.string.tip_wifi_disconnect : C2663R.string.tip_ble_disconnect));
            } else if (OtherModel.this.saveDis != null && !OtherModel.this.saveDis.isDisposed()) {
            } else {
                if (OtherModel.this.name.getValue() == null || TextUtils.isEmpty(OtherModel.this.name.getValue().trim())) {
                    OtherModel.this.showFailDialog(Utils.getString(C2420R.string.tip_empty_name));
                    OtherModel.this.name.setValue(OtherModel.this.originName);
                } else if (ProtocolTransformer.isWorkWiFi(OtherModel.this.deviceType, OtherModel.this.connectType)) {
                    OtherModel.this.saveForNet();
                } else {
                    OtherModel.this.saveForBle();
                }
            }
        }
    });
    ...
    public MutableLiveData<String> step1Text = new MutableLiveData<>();
    public MutableLiveData<String> step2Text = new MutableLiveData<>();
    public MutableLiveData<String> step3Text = new MutableLiveData<>();
    public MutableLiveData<Boolean> step3Visible = new MutableLiveData<>();
    ...
    public MutableLiveData<String> vpdInfo = new MutableLiveData<>();
    ...
    public OtherModel(Application application) {
        super(application);
        ...
        if (this.disClickable.getValue() != Boolean.TRUE) {
            ...
            observable.subscribeOn(Schedulers.m983io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new DataObserver<Void>() {
                public void doOnSubscribe(Disposable disposable) {
                    OtherModel.this.showDialog((String) null, disposable);
                    OtherModel.this.saveDis = disposable;
                }

                /* access modifiers changed from: protected */
                public void onError(String str) {
                    RxBus.getDefault().post(new ProgressEvent((byte) 1));
                    OtherModel.this.showFailDialog(str);
                    OtherModel.this.disClickable.setValue(false);
                }

                /* access modifiers changed from: protected */
                public void onSuccess(Void voidR) {
                    RxBus.getDefault().post(new ProgressEvent((byte) 1));
                    Notification unused = OtherModel.this.notification = create;
                    OtherModel.this.repository.setNotification(OtherModel.this.notification, z);
                    OtherModel.this.addSubscribe(Maybe.just(true).delay(1500, TimeUnit.MILLISECONDS).subscribe(new Consumer<Boolean>() {
                        public void accept(Boolean bool) throws Exception {
                            OtherModel.this.back();
                            OtherModel.this.dismissDialog();
                        }
                    }));
                }
            });
            ...
            removeForBle(new Consumer<Boolean>() {
                public void accept(Boolean bool) throws Exception {
                    if (bool.booleanValue()) {
                        OtherModel.this.saveForBle(create, true);
                    }
                }
            });
        }
    ...
    public void saveForBle(final Notification notification2, boolean z) {
        ...
        if (connect != null) {
            ...
            this.saveDis = single.compose(RxUtils.bindToLifecycle(getLifecycleProvider())).subscribeOn(Schedulers.m983io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<Boolean>() {
                public void accept(Boolean bool) throws Exception {
                    if (bool.booleanValue()) {
                        Notification unused = OtherModel.this.notification = notification2;
                        OtherModel.this.back();
                    }
                }
            });
    ...
    public void init(NotificationRepository notificationRepository, String str, String str2, byte b, byte b2, Notification notification2, boolean z, byte b3, byte b4, byte b5) {
        ...
        Messenger.getDefault().sendNoMsg(OtherFragment.UPDATE_PARAM);
    }
    ...
    private void removeForBle(final Consumer<Boolean> consumer) {
        BleStatue connect = this.repository.getConnect(this.notification.mac);
        if (connect != null) {
            addSubscribe(this.repository.removeNotification(connect, this.notification, false).compose(RxUtils.bindToLifecycle(getLifecycleProvider())).subscribeOn(Schedulers.m983io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<Boolean>() {
                public void accept(Boolean bool) throws Exception {
