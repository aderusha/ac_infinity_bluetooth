...
public class SlideRecyclerView extends RecyclerView {
    ...
    public boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            float f2 = (float) x;
            boolean z3 = Math.abs(f2 - this.mFirstX) >= ((float) this.mTouchSlop) && Math.abs(f2 - this.mFirstX) > Math.abs(((float) y) - this.mFirstY);
            ViewGroup viewGroup3 = this.mFlingView;
            if (!(viewGroup3 == null || viewGroup3.getScrollX() == 0)) {
                z = true;
            }
            if ((z2 || z3) && (z || f2 < this.mFirstX)) {
                this.mIsSlide = true;
                return true;
            }
        }
