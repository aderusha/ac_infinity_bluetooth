...
public class HistoryEParse extends BaseParse<History> {
    ...
    public History parse() {
        History parseHistory = EFamilialResolution.parseHistory(this.now, this.f141i, (this.start / 60) * 60, this.type, this.version);
        this.start += (long) this.historySize;
        this.total--;
        if (this.type == 2) {
            this.f141i += this.packetSize;
        } else {
            this.f141i = this.f141i + this.packetSize + this.now[this.f141i + this.packetSize] + 1;
        }
        return parseHistory;
    }
