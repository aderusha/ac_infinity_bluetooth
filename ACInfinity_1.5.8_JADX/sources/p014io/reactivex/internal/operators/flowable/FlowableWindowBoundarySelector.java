...
import p014io.reactivex.subscribers.DisposableSubscriber;
...
public final class FlowableWindowBoundarySelector<T, B, V> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    static final class WindowBoundaryMainSubscriber<T, B, V> extends QueueDrainSubscriber<T, Object, Flowable<T>> implements Subscription {
        ...
        /* renamed from: ws */
        final List<UnicastProcessor<T>> f1126ws;
        ...
        public void onNext(T t) {
            if (!this.done) {
                if (fastEnter()) {
                    for (UnicastProcessor<T> onNext : this.f1126ws) {
                        onNext.onNext(t);
                    }
                    if (leave(-1) == 0) {
                        return;
        ...
        public void drainLoop() {
            ...
            List<UnicastProcessor<T>> list = this.f1126ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (z && z2) {
                    ...
                    if (windowOperation.f1127w != null) {
                        if (list.remove(windowOperation.f1127w)) {
                            windowOperation.f1127w.onComplete();
                            if (this.windows.decrementAndGet() == 0) {
                                dispose();
                                return;
                            }
        ...
        public void close(OperatorWindowBoundaryCloseSubscriber<T, V> operatorWindowBoundaryCloseSubscriber) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseSubscriber.f1125w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f1127w;

        WindowOperation(UnicastProcessor<T> unicastProcessor, B b) {
            this.f1127w = unicastProcessor;
            this.open = b;
        }
    }
    ...
    static final class OperatorWindowBoundaryCloseSubscriber<T, V> extends DisposableSubscriber<V> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f1125w;
